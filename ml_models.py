...
@st.cache_resource
def plot_forecasting_arima(ts, index_type):
    ...
    fig.update_layout(title=f'{index_type} ARIMA Forecast (24-Month Horizon)', xaxis_title='Year', yaxis_title=f'{index_type} Value', legend=dict(x=0.01, y=0.99, bordercolor="Black", borderwidth=1), template='plotly_white', font=dict(size=14), height=600)
    return fig

@st.cache_resource
def plot_forecasting_prophet(ts, index_type):
...
@st.cache_resource
def plot_forecasting_both(ts, index_type):
    with st.spinner("Running ARIMA & Prophet, calculating performance metrics... This may take a moment."):
        # --- 1. Calculate Performance Metrics on a Test Set ---
        train = ts[:-12]
        test = ts[-12:]

        # Fit ARIMA on train, predict test
        arima_model_eval = pm.auto_arima(train.dropna(), start_p=1, start_q=1, test='adf', max_p=3, max_q=3, m=12, d=None, seasonal=True, start_P=0, D=1, trace=False, error_action='ignore', suppress_warnings=True, stepwise=True)
        ...
        # ARIMA
        arima_model_final = pm.auto_arima(ts.dropna(), start_p=1, start_q=1, test='adf', max_p=3, max_q=3, m=12, d=None, seasonal=True, start_P=0, D=1, trace=False, error_action='ignore', suppress_warnings=True, stepwise=True)
        n_periods = 24
        arima_forecast, arima_conf_int = arima_model_final.predict(n_periods=n_periods, return_conf_int=True)
        arima_forecast_index = pd.date_range(start=ts.index[-1], periods=n_periods + 1, freq='MS')[1:]
        ...
        prophet_forecast_future = prophet_forecast[prophet_forecast['ds'] > ts.index.max()]
    ...
    fig.update_layout(title=f'{index_type} ARIMA vs. Prophet Forecast (24-Month Horizon)', xaxis_title='Year', yaxis_title=f'{index_type} Value', legend=dict(x=0.01, y=0.99, bordercolor="Black", borderwidth=1), template='plotly_white', font=dict(size=14), height=600)
