...
def create_map(gdf, data, index_type, date):
    ...
    map_data = data[(data['index_type'] == index_type) & (data['date'].dt.strftime('%Y-%m') == date.strftime('%Y-%m'))]
    ...
    # Add tooltips
    style_function = lambda x: {'fillColor': x['properties']['color'], 'color':'black', 'fillOpacity': 0.7, 'weight': 0.2}
    folium.GeoJson(
        merged_gdf,
